...
说明：`paidAt`、`shippedAt`、`completedAt` 为时间戳字符串（ISO-8601），对应状态未发生时字段可省略（无 null 值）；契约上允许 nullable。`paidAt` 在支付成功后填充，`shippedAt` 在团长发货后填充，`completedAt` 在买家确认收货后填充。
...
- 默认 `SANDBOX_PAYMENT_ENABLED=false` 时，统一支付入口复用本地模拟支付闭环，返回 `mode=simulate` 和更新后的订单。
- `SANDBOX_PAYMENT_ENABLED=true` 时，统一支付入口返回 `mode=sandboxAlipay` 和支付宝沙箱 H5 自动提交表单 HTML；订单不会在发起支付时变更为已支付。
- `payAmount=0` 的订单直接完成本地支付，返回 `mode=zeroAmount` 和更新后的订单。
- 支付宝沙箱支付成功以 `POST /api/v1/payments/alipay/sandbox/notify` 异步通知验签成功为准；同步回跳只用于前端刷新订单状态。
- 支付成功后复用同一事务落账逻辑：扣减 `groupStock`、增加 `soldCount`、核销优惠券、累计会员成长、发送站内通知并写入订单聊天事件。
...
```http
POST /api/v1/payments/alipay/sandbox/notify
Content-Type: application/x-www-form-urlencoded
```

服务端验签并校验 `app_id`、`out_trade_no`、`total_amount`、`trade_status`；`TRADE_SUCCESS` / `TRADE_FINISHED` 返回纯文本 `success`，否则返回 `failure`。
...
该接口保留给本地开发、自动化测试和历史兼容，前端主流程应调用 `/pay`。
...
- 本轮仅接入支付宝沙箱 H5 支付，不接真实微信支付、真实退款、对账和支付流水表。
...
| 订单待支付 -> 已支付 | `POST /api/v1/orders/{orderId}/pay` 或兼容接口 `POST /api/v1/orders/{orderId}/simulate-pay` |
...
sequenceDiagram
    ...
    participant ALI as Alipay Sandbox
    participant O as orders
    participant OI as order_items
    participant GI as group_buy_items
    participant M as member_relations

    C->>API: POST /api/v1/orders/{orderId}/pay
    API->>O: 查询订单并校验归属当前用户
    alt 订单已支付
        API-->>C: ORDER_ALREADY_PAID 或当前订单
    else 订单不是 pendingPay/unpaid
        API-->>C: ORDER_NOT_PAYABLE
    else SANDBOX_PAYMENT_ENABLED=true 且 payAmount>0
        API-->>C: mode=sandboxAlipay + formHtml
        C->>ALI: 提交支付宝 H5 表单
        ALI->>API: POST /payments/alipay/sandbox/notify
        API->>API: 验签并校验金额
        API->>API: 执行支付落账事务
