# P1 Batch 5：售后退款和 P1 回归冻结

## 1. 目标

补齐售后退款闭环，并完成 P1 后端接口、状态、文档和测试回归冻结。
...
售后申请、审核、模拟退款形成完整闭环；订单列表和详情能展示售后 / 已退款状态；退款不会重复执行；P1 所有新增接口与文档一致，MVP 主链路仍可回归通过。

## 8. 联调文档更新项

新增“售后申请”“团长售后处理”“模拟退款”“P1 全链路回归”链路；更新订单状态、支付状态、优惠券售后口径、会员成长冲正口径、库存返还口径和测试数据准备方式。
